...
#'
#' The function searches a query MS/MS spectrum in a spectral library
#'
...
#' @param tops Integer. Number of top compound candidates kept.
...
library_search_engine <- function(library_type = c("Metabolite", "Private", "Public", "Local"), 
                         local_library = NULL, query_spectrum=NULL, ion.mode = c("Positive", "Negative"), 
  ...
  if (library_type=="Metabolite"){
    data("METABOLITE_PLUS")
    library = library22
    library$metadata$SCANS = 1:nrow(library$metadata)
    api_feature = metabolite_feature
    api_profile = metabolite_profile
  }
